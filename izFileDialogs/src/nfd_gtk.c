...
nfdresult_t NFD_OpenDialogMultiple2( const nfdchar_t *filterList,
                                    const nfdchar_t *defaultPath,
                                    nfdpathset_t *outPaths )
{
    Window x11ParentWin;
    ...
    iznWin x11parentWin = iznGetActiveWin();
	  disp = gdk_display_get_default();
	  parentWin = gdk_x11_window_foreign_new_for_display(disp, x11ParentWin);
	  dialog = gtk_file_chooser_dialog_new( "Open Files",
                                          parentWin,
                                          GTK_FILE_CHOOSER_ACTION_OPEN,
                                          "_Cancel", GTK_RESPONSE_CANCEL,
