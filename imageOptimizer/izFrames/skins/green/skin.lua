...
-- controls 
skin.controls = {}
skin.controls.smallfont = love.graphics.newFont("resources/dejaVuSansRegular.ttf", 16)
...
skin.controls.multichoicerow_body_hover_color = {0, 0.478, 1, 1}
...
skin.controls.textinput_highlight_bar_color = {0, 0.478, 1, 1}
skin.controls.textinput_text_font = skin.controls.smallfont
...
skin.controls.columnlistrow_body_selected_color = {0, 0.478, 1, 1}
...
skin.controls.columnlistrow_text_font = skin.controls.smallfont
...
skin.controls.link_text_down_color = {0, 0.478, 1, 1}
skin.controls.link_text_nohover_color = {0, 0, 0, 1}
skin.controls.link_text_hover_color = {0, 0.478, 1, 1}
skin.controls.link_text_nonclickable_color = {0.424, 0.459, 0.494, 1}
skin.controls.link_text_font = skin.controls.smallfont
...
function skin.frame(object)
	...
	if icon then
		...
		skin.PrintText(name, x + iconwidth + 10, y + 2)
	else
		love.graphics.setColor(namecolor)
		skin.PrintText(name, x + 5, y + 2)
	end
		...
		if inlist then
			...
			if (y + texty) <= (listy + listhieght) and y + ((texty + theight)) >= listy then
				...
				if shadow then
					...
					skin.PrintText(text, x + textx + shadowxoffset, y + texty + shadowyoffset)
				end
				if link then
...
function skin.textinput(object)
	...
	local width = object:GetWidth()
	local height = object:GetHeight()
	local font = skin.controls.textinput_text_font
...
function skin.link(object)
    ...
    local down = object:GetDown()
    local enabled = object:GetEnabled()
    local textdowncolor = skin.controls.link_text_down_color
    local texthovercolor = skin.controls.link_text_hover_color
    local textnohovercolor = skin.controls.link_text_nohover_color
    local textnonclickablecolor = skin.controls.link_text_nonclickable_color

    love.graphics.setFont(font)

    if not enabled then
        love.graphics.setColor(textnonclickablecolor)
        skin.PrintText(text, x, y)
    elseif hover then
       if down then
            love.graphics.setColor(textdowncolor)
            skin.PrintText(text, x, y)
